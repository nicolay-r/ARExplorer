...
# ---------------------------------------------------------------------------
# after_model_callback: guarantee a non-empty model turn
# ---------------------------------------------------------------------------
...
def ensure_nonempty_response(
    *,
    ...
    `gemini-2.5-flash-lite` intermittently ends a turn with no content and no
    tool call (``finishReason: STOP`` with zero output tokens). Left alone that
    surfaces as a blank reply in EVERY runner — the custom server, `adk web`,
    and `adk run` alike. Handling it here, at the agent level, guarantees the
    user always sees something no matter how the agent is served.
